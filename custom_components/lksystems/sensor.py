...
async def async_setup_entry(
        ...
        for hub_id, hub_data in coordinator.data["hub_data"].items():
            ...
            if isinstance(hub_id, str) and ":" in hub_id:
                ...
                for device in coordinator.data.get("devices", []):
                    if device.get("mac") == hub_id or device.get("deviceTitle", {}).get("identity") == hub_id:
                        device_title = device.get("deviceTitle", {})
                        hub_map[hub_id] = {
                            "device": device,
    ...
    if coordinator.data and "devices" in coordinator.data:
        for device in coordinator.data["devices"]:
            ...
            if parent_identity and parent_identity in hub_map:
                ...
                _LOGGER.debug("Device %s belongs to hub %s", device_identity, parent_identity)
            ...
            if isinstance(hub_data, dict) and "devices" in hub_data:
                for device in hub_data["devices"]:
                    ...
                    if parent_identity in hub_map:
                        ...
                        # Map device to its parent hub
                        device_to_hub_map[device_identity] = parent_identity
                        _LOGGER.debug("Device %s from hub_data belongs to hub %s", device_identity, parent_identity)
    ...
    for hub_identity, hub_info in hub_map.items():
        ...
        if entity_id not in created_entity_ids:
            ...
            _LOGGER.debug("Created hub entity: %s with %d children", entity_id, len(hub_info["children"]))
    ...
    if coordinator.data and "devices" in coordinator.data:
        for device in coordinator.data["devices"]:
            ...
            # Process sensor data
            if device_title.get("deviceGroup") == "arc" and device_title.get("deviceType") == "arc-sense":
                device_id = device.get("mac")
                device_identity = device_title.get("identity") or device_id
                ...
                if "measurement" in device and device["measurement"].get("currentTemperature") is not None:
                ...
                # Add humidity entity
                if "measurement" in device and device["measurement"].get("currentHumidity") is not None:
                ...
                # Add battery entity
                if "measurement" in device and device["measurement"].get("currentBattery") is not None:
                ...
                # Add RSSI entity
                if "measurement" in device and device["measurement"].get("currentRssi") is not None:
...
class LKSensorEntity(CoordinatorEntity, SensorEntity):
    ...
    def __init__(
        ...
        self._is_thermostat = (
            device_title.get("deviceGroup") == "arc" and
        ...
        if friendly_name:
            ...
            room_name = device_title.get("zone", {}).get("zoneName") if device_title.get("zone") else None
            if room_name:
        ...
        if parent_identity:
            ...
            _LOGGER.debug("Device %s connected via %s", device_identity, parent_identity)
    ...
    def native_value(self) -> Any:
        ...
        if "device_details" in self.coordinator.data:
            device_details = self.coordinator.data["device_details"].get(self._device_identity)
            if device_details and "measurement" in device_details:
                ...
                if self._entity_key == "temperature":
                    ...
                    if temp_value is not None:
                        _LOGGER.debug("Using temperature from direct measurement: %s for %s", temp_value, self._device_identity)
                    ...
                    if humid_value is not None:
                        _LOGGER.debug("Using humidity from direct measurement: %s for %s", humid_value, self._device_identity)
                        return float(humid_value) / 10
                elif self._entity_key == "battery":
                    battery = measurement.get("currentBattery")
                    if battery is not None:
                        _LOGGER.debug("Using battery from direct measurement: %s for %s", battery, self._device_identity)
                        return battery
                elif self._entity_key == "rssi":
                    rssi = measurement.get("currentRssi")
                    if rssi is not None:
                        _LOGGER.debug("Using RSSI from direct measurement: %s for %s", rssi, self._device_identity)
                        return rssi
                elif self._entity_key == "desired_temperature":
                    temp_value = measurement.get("desiredTemperature")
                    if temp_value is not None:
                        _LOGGER.debug("Using desired temp from direct measurement: %s for %s", temp_value, self._device_identity)
                        return float(temp_value) / 10
    ...
    async def async_update(self) -> None:
        ...
        _LOGGER.warning("Force updating entity: %s (%s)", self._attr_name, self._device_identity)
        ...
        if hasattr(self.coordinator, "force_device_update"):
            try:
                ...
                _LOGGER.warning("Manual update completed for device: %s", self._device_identity)
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        _LOGGER.info("Handling coordinator update for %s (%s)", self._attr_name, self._device_identity)
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if self.coordinator and hasattr(self.coordinator, "update_interval") and hasattr(self.coordinator, "_last_update_time"):
            next_update = self.coordinator._last_update_time + self.coordinator.update_interval
...
class LKArcHubEntity(CoordinatorEntity, SensorEntity):
    ...
    def __init__(
        ...
        if parent_identity:
            device_info["via_device"] = (DOMAIN, parent_identity)
            _LOGGER.debug("Hub device %s connected via %s", device_identity, parent_identity)
                            ...
                            if self._entity_key == "status":
                                if "measurement" in device:
                                    ...
                                    if device_title.get("deviceRole") == "arc-tune":
                                        _LOGGER.debug("Found thermostat device %s in hub %s with state: %s", 
                                                     self._device_identity, hub_id, 
                                                     device["measurement"].get("connectionState", "Connected"))
