...
from .pylksystems import LKSystemsManager, LKSystemsError
...
def is_token_valid(token: str) -> bool:
    ...
    try:
        ...
        _LOGGER.debug("Token validity check: exp=%s, now=%s, valid=%s", 
                     exp_time, current_time, is_valid)
        return is_valid
...
class LKSystemCoordinator(DataUpdateCoordinator[LkStructureResp]):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        update_interval_minutes = int(entry.data.get(CONF_UPDATE_INTERVAL, DEFAULT_UPDATE_INTERVAL))
    ...
    def _setup_update_interval(self):
        ...
        _LOGGER.warning(f"Setting up update interval for {DOMAIN} to {self._update_interval_minutes} minutes")
        ...
        _LOGGER.warning(f"Next automatic update scheduled for: {next_update.isoformat()}")
    ...
    async def set_thermostat_temperature(self, device_id, temperature):
        ...
        try:
            ...
            async with LKSystemsManager(username, password) as lk_inst:
                ...
                result = await lk_inst.set_thermostat_temperature(device_id, temperature)
    ...
    async def force_device_update(self, device_id: str) -> bool:
        ...
        try:
            ...
            async with LKSystemsManager(username, password) as lk_inst:
                ...
                success = await lk_inst.get_device_measurement(device_id, force_update=True)
                ...
                if success and device_id in lk_inst.device_measurements:
                    ...
                    if self.data:
                        ...
                        self.data["device_details"][device_id]["measurement"] = measurement_data
                        
                        # Also update in devices list
                        for device in self.data.get("devices", []):
                            device_title = device.get("deviceTitle", {})
                            if (device.get("mac") == device_id or 
                                device_title.get("identity") == device_id):
    ...
    async def _async_update_data(self) -> LkStructureResp:  # noqa: C901
        ...
        self._last_update_time = dt_util.now()
        _LOGGER.info("Starting LK Systems data update at %s", self._last_update_time.isoformat())
        ...
        try:
            ...
            if not username or not password:
                _LOGGER.error("Missing credentials for LK Systems API. Check your configuration.")
            ...
            async with LKSystemsManager(username, password) as lk_inst:
                ...
                if stored_jwt and is_token_valid(stored_jwt):
                    ...
                    TOKEN_STORAGE[self._entry_id] = {
                        "jwt": lk_inst.jwt_token,
                        "refresh": lk_inst.refresh_token,
                        "expiry": dt_util.utcnow().timestamp() + 3600  # Assume 1 hour validity
                    }
                ...
                resp: LkStructureResp = {
                    ...
                    "next_update_time": (self._last_update_time + self.update_interval).isoformat(),
                }
                ...
                if "realestateMachines" in lk_inst.user_structure:
                    for machine in lk_inst.user_structure["realestateMachines"]:
                        ...
                        device_data = {
                            ...
                            "cacheUpdated": lk_inst.user_structure.get("cacheUpdated", 0)
                        }
                        devices.append(device_data)
                        
                        # Track Arc sense devices for direct measurements
                        if (machine.get("deviceGroup") == "arc" and 
                            machine.get("deviceType") == "arc-sense"):
                        ...
                        if machine.get("deviceGroup") == "arc":
                            if machine.get("deviceType") == "arc-sense":
                                ...
                                if await lk_inst.get_device_measurement(device_identity, force_update=True):
                                    resp["device_details"][device_identity] = {
                                        "measurement": lk_inst.device_measurements.get(device_identity)
                                    }
                                    # Also add to the device in the devices list
                                    device_data["measurement"] = lk_inst.device_measurements.get(device_identity)
                                
                                # Fetch configuration data
                                if await lk_inst.get_device_configuration(device_identity):
                                    if device_identity not in resp["device_details"]:
                                        resp["device_details"][device_identity] = {}
                                    resp["device_details"][device_identity]["configuration"] = (
                                        lk_inst.device_configurations.get(device_identity)
                                    )
                                    # Also add to the device in the devices list
                                    device_data["configuration"] = lk_inst.device_configurations.get(device_identity)
                                ...
                                if await lk_inst.get_hub_devices(hub_id):
                                    ...
                                    if isinstance(lk_inst.hub_devices, dict) and "devices" in lk_inst.hub_devices:
                                        for hub_device in lk_inst.hub_devices["devices"]:
                                            if hub_device.get("mac") and hub_device.get("mac") not in device_identities:
                                                device_identities.append(hub_device.get("mac"))
                                                ...
                                                if device_mac:
                                                    ...
                                                    if "measurement" in hub_device:
                                                        if device_mac not in resp["device_details"]:
                                                            resp["device_details"][device_mac] = {}
                                                        resp["device_details"][device_mac]["measurement"] = hub_device["measurement"]
                        
                        # For cubic devices (if they exist)
                        elif machine.get("deviceType") == "cubicsecure" and machine.get("deviceRole") == "cubicsecure":
                            ...
                            try:
                                if await lk_inst.get_cubic_secure_messurement(device_identity):
                                    resp["cubic_last_messurement"] = lk_inst.cubic_secure_messurement
                            except Exception as err:
                                _LOGGER.warning("Error fetching cubic measurements: %s", str(err))
                
                # Now directly fetch fresh measurement data for each Arc sense device
                _LOGGER.info("Fetching direct measurements for %d Arc sense devices", len(arc_sense_devices))
                for device_id in arc_sense_devices:
                    _LOGGER.info("Fetching fresh measurement data for Arc device: %s", device_id)
                    
                    # Always get the latest data with force_update=True
                    if await lk_inst.get_device_measurement(device_id, force_update=True):
                        ...
                        if measurement_data:
                            ...
                            resp["device_details"][device_id]["measurement"] = measurement_data
                            
                            # Log the fetched values
                            _LOGGER.info(
                                "Got measurement for %s: Temp=%.1f°C, Humidity=%.1f%%, Battery=%s%%, RSSI=%sdBm", 
                                device_id,
                                float(measurement_data.get("currentTemperature", 0)) / 10,
                        ...
                        _LOGGER.warning("Failed to get measurement data for device %s", device_id)
                        ...
                        if isinstance(hub_data, dict) and "devices" in hub_data:
                            for device in hub_data["devices"]:
                                ...
                                if (device_title.get("deviceGroup") == "arc" and 
                                    device_title.get("deviceType") == "arc-sense"):
                                    
                                    _LOGGER.info("Fetching fresh measurement for hub device: %s", device_id)
                                    ...
                                    if await lk_inst.get_device_measurement(device_id, force_update=True):
                                        ...
                                        _LOGGER.warning("Failed to get measurement for hub device %s", device_id)
